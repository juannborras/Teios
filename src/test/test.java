...
public class test {
    ...
    public static void main(String[] args) {
        try {
            ...
            Path dataDir = Path.of("./data");   // lo seguimos usando SOLO para producto_proveedor.csv
            Files.createDirectories(dataDir);

            // AHORA: repositorios contra la BD
            ...
            // Servicio de pedidos (igual que antes)
            IServicioPedidos svcPedidos = new ServicioPedidos(repoProductos, repoPedidos, repoProveedores);

            // ESTA CONFIG SIGUE SIENDO CSV (es más un “config” que datos de negocio)
            ProveedorProductoConfig provCfg = new ProveedorProductoConfig(dataDir.resolve("producto_proveedor.csv"));

            // Servicio de inventario usando ya repos JDBC
            IServicioInventario svcInv = new ServicioInventario(repoProductos, repoVentas, svcPedidos, provCfg);
