 ...
 * Comportamiento y contratos importantes:
  - Esta implementación almacena solo datos básicos en los CSV. La resolución
    ...
    `Producto`) se delega a capas superiores (p. ej. ServicioPedidos).
...
public class RepositorioPedidoCSV implements IRepositorioPedido {
    ...
    @Override public List<PedidoReposicion> listar() {
        // Para reconstruir items necesitamos un resolver de Producto/Proveedor en ServicioPedidos.
        ...
        try (var br = Files.newBufferedReader(pedidos)) {
            ...
            for (String line; (line = br.readLine()) != null; ) {
                ...
                encabezados.add(
                        PedidoReposicion.reconstruir(id, new Proveedor(provId, "N/D", "N/D"),
                                fecha, null, null, estado)
                );
            }
        } catch (IOException e){ throw new UncheckedIOException(e); }
        return encabezados;
